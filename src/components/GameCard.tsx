...
export const GameCard: React.FC<GameCardProps> = ({
  ...
  return (
    <div className={`relative group glass-panel rounded-3xl p-6 border transition-all duration-300 flex flex-col justify-between overflow-hidden ${
      isDaily
        ? 'border-amber-500/40 bg-gradient-to-b from-stadium-900/90 via-stadium-950 to-pitch-950/60 shadow-glow-gold'
        : 'border-slate-800/80 hover:border-emerald-500/50 hover:shadow-glow-emerald'
    }`}>
      
      {/* Background Subtle Accent Effect */}
      <div className="absolute -top-16 -right-16 w-32 h-32 bg-emerald-500/10 rounded-full blur-2xl group-hover:bg-emerald-500/20 transition-all duration-500 pointer-events-none" />

      <div>
        {/* Header Tag & Icon */}
        <div className="flex items-center justify-between mb-4">
          <div className={`p-3.5 rounded-2xl ${
            isDaily ? 'bg-amber-500/10 text-amber-400 border border-amber-500/30' : 'bg-pitch-900/60 text-emerald-400 border border-emerald-500/30'
          }`}>
          ...
          <span className={`px-3 py-1 rounded-full text-xs font-bold font-display uppercase tracking-wider ${
            isDaily
              ...
              : 'bg-pitch-950 text-emerald-400 border border-emerald-500/30'
          }`}>
            {tag}
          </span>
        </div>
      ...
      <div className="mt-6 pt-4 border-t border-slate-800/60 flex items-center justify-between">
        ...
        <button
          onClick={() => onPlay(id)}
          className={`flex items-center gap-2 px-5 py-2.5 rounded-xl font-bold text-sm font-display transition-all duration-200 shadow-md ${
            isDaily
